...
//		System.out.println("GET요청 정상적으로 받았습니다.");
		...
		String name = request.getParameter("name"); // "구현서" | ""
		String gender = request.getParameter("gender"); // "M" | "W" | null
		int age = Integer.parseInt(request.getParameter("age")); // "315" => 23
		...
		//장점 : java코드 내에 작성하기 때문에 반복문이나 조건문, 유용한 메소드 같은 것들을 활용하여 동적인 응답을 내릴 수 있다.
		//단점 : 불편, 복잡, 혹시라도 차후에 html을 수정한다면 java코드를 수정하는 것이기 때문에 수정된 내용을 다시 반영시키고자 서버를 재실행 해야된다.
		...
		//3) 위에서 가져온 스트림을 통해서 응답html구문을 한줄씩 출력
		out.println("<html>");
